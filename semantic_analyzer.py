...
"""
################################################################################################
# Edit the file name for examination here.
filename = "examples/example1.txt"

# Output file
import sys
f = open(filename[:-4]+'_result.log','a')
sys.stdout = f
...
################################################################################################
def check_function_args(func):
    print('ENTER <check_function_args>\n')
    this_args = function_dic[func]
    if not this_args:
        print('EXIT <check_function_args>\n')
        return
    while this_args:
        [this_id],_ = assign()
        if this_id==this_args[0]:
            this_args.pop(0)
        else:
            print('Error detected. The function args are wrong.')
            error()
            return
    print("EXIT <check_function_args>\n")    
    return
...
if s:
    ...
    print(static_vars)      
    if syntax_flag:
        print("No Semantic Error Detected.")